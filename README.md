...
The Action Bridge uses one direction-neutral Binding for both runtime
directions. The entry point name describes the ROS-facing role:

| Entry point | ROS role | Hakoniwa PDU-RPC role | Endpoint used |
| --- | --- | --- | --- |
...
The canonical schema and Fibonacci example are:
...
Generate the shared Hakoniwa Action runtime configuration once:
...
The same Binding and generated files can then be used in either direction:

```bash
# Expose a Hakoniwa Action Server as a ROS 2 Action Server.
action-server \
  --config config/action/fibonacci.json \
  --output-dir build/generated/action/fibonacci \
  --rpc-library /path/to/libhakoniwa_pdu_rpc.so

# Forward a Hakoniwa Action Client to a ROS 2 Action Server.
action-client \
  --config config/action/fibonacci.json \
  --output-dir build/generated/action/fibonacci \
  --rpc-library /path/to/libhakoniwa_pdu_rpc.so
...
When `--rpc-library` is omitted, `HAKO_PDU_RPC_LIBRARY` is used. When
`--output-dir` is omitted, the generator uses
`build/generated/action/<binding-id>`. A Business Pack Recipe should pass its
Recipe-local config directory explicitly.

Users specify only the semantic binding and transport files. Channel IDs,
packet names, slot routing, and resolved Action runtime files are generated by
the PDU-RPC generator contract. Raw packets, headers, slots, and protocol state
remain inside `hakoniwa-pdu-rpc`; this package maps typed Goal, Feedback, and
Result bodies to ROS messages and correlates the two Goal handles.

The Docker native suite covers both directions over real TCP. For the Action
Client Bridge direction it starts a ROS Fibonacci Action Server and verifies
Hakoniwa Goal acceptance/rejection, Feedback, terminal Results, and accepted
and rejected Cancel requests:
